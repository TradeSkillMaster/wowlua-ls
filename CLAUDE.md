...
- `NarrowKind::NumCompare { op, bound }` (numeric case discrimination) — a `if x > 1` style guard on a bare symbol records `(Operator, bound_string)` in `Analysis::num_compare_narrowed_symbols` (detected in `narrowing.rs`, oriented via `flip_comparison` when the literal is on the LHS). `narrow_kind_for` returns it only as a **fallback** (after StripNil/StripFalsy/StripTruthy/ClassEq) so a compound `x and x > 0` keeps the stronger nil-stripping. In `overload_type_compatible_with` → `overload_type_survives_num_compare` (resolve.rs), a tuple-union case's `NumberLiteral` slot is eliminated only when `value <op> bound` is provably false (parsed via `parse_num_literal_str`); plain `number`/`any` slots always survive (no range tracking). Does NOT strip nil from the symbol itself.
