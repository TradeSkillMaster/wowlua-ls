...
**Exhaustive diagnostic checking**: The harness fails on any WARNING/ERROR diagnostic that is not covered by a `diag:` assertion. This catches diagnostic regressions automatically — if a code change introduces a new unexpected diagnostic, the test fails even without an explicit `diag: none` on that line. Therefore, **do not write `diag: none`** — it is redundant since the absence of an unexpected diagnostic is already verified by the harness. Only use `diag: <code>` to assert that a specific diagnostic IS emitted. HINT-level diagnostics (e.g. `unused-local`, `unused-function`, `shadowed-local`) and syntax errors are exempt, since they are noisy in test code that creates locals/functions just for hover/def assertions. To suppress expected WARNING/ERROR diagnostics in test files that aren't testing those specific codes, use `---@diagnostic disable: code1, code2` at the top of the file.
