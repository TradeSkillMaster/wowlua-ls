...
- `src/annotations/` — Annotation system (types, parsing, cross-file scanning):
  ...
  - `scan_globals.rs` — `scan_file_globals[_with_synth]()`, `scan_dynamic_global_prefixes()` (detects `_G["PREFIX"..k]` patterns for auto-allowed globals), workspace-level `synthesize_return_only_overloads_for_body()`, and private synth_* helpers
