...
- `tests/allowed-globals/` — Allowed globals via `.wowluarc.json` config (`globals.read`/`globals.write`), `create-global` diagnostic, `SLASH_*` auto-detection (default `allow_slash_commands: true`), and `BINDING_HEADER_*`/`BINDING_NAME_*` auto-detection (default `allow_binding_globals: true`)
