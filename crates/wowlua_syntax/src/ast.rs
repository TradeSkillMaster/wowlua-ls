...
impl<'a> Identifier<'a> {
    ...
    pub fn has_non_string_bracket_tail(&self) -> bool {
    ...
    pub fn has_prefix_expr_base(&self) -> bool {
        matches!(
            self.node.kind(),
    ...
    pub fn has_non_string_bracket_in_chain(&self) -> bool {
    ...
    pub fn names_with_brackets(&self) -> Vec<String> {
        let mut result = Vec::new();
    ...
    pub fn has_any_dynamic_bracket(&self) -> bool {
    ...
    pub fn has_complex_dynamic_bracket(&self) -> bool {
...
impl<'a> BinaryExpression<'a> {
    ...
    pub fn op_token_range(&self) -> Option<crate::syntax::TextRange> {
        self.node.children_with_tokens()
...
pub fn extract_bracket_string_key(node: SyntaxNode<'_>) -> Option<String> {
...
pub fn extract_bracket_variable_key(node: SyntaxNode<'_>) -> Option<String> {
    let mut seen_bracket = false;
    for child in node.children_with_tokens() {
        match child {
            NodeOrToken::Token(t) if t.kind() == SyntaxKind::LeftSquareBracket => {
...
pub fn extract_bracket_concat_string_literal(node: SyntaxNode<'_>) -> Option<(String, bool)> {
    let mut seen_bracket = false;
    for child in node.children_with_tokens() {
        match child {
            NodeOrToken::Token(t) if t.kind() == SyntaxKind::LeftSquareBracket => {
...
pub fn extract_bracket_literal_key(node: SyntaxNode<'_>) -> Option<String> {
    let mut seen_bracket = false;
    for child in node.children_with_tokens() {
        match child {
            NodeOrToken::Token(t) if t.kind() == SyntaxKind::LeftSquareBracket => {
