...
// the IR types embed it. Re-exported here so the parsing/scanning code below —
// and every `crate::annotations::AnnotationType` use site across the crate —
// keeps referring to it at this path.
pub use wowlua_core::annotations::{
    AnnotationType, KEYOF_SELF_TARGET, ParamInfo, TuplePosition, Visibility,
};
...
pub fn collect_referenced_type_names(ann: &AnnotationType, out: &mut HashSet<String>) {
    match ann {
...
/// modifier is present.
pub fn strip_class_modifier(s: &str) -> &str {
...
pub fn class_referenced_names(c: &ClassDecl, out: &mut HashSet<String>) {
...
pub fn global_referenced_names(g: &ExternalGlobal, out: &mut HashSet<String>) {
    for p in &g.params {
        collect_referenced_type_names(&p.typ, out);
    }
    for r in &g.returns {
...
pub fn parse_keyof_constraint(raw: &str) -> Option<&str> {
...
pub fn parent_link_with_bindings(parent_name: &str) -> Option<(String, Vec<AnnotationType>)> {
...
pub fn extract_overload_self_return(
...
pub struct SelfFieldEntry {
    ...
    pub deferred_call_range: Option<(u32, u32)>,
}
...
pub fn lower_tuple_form_cases<F>(
...
impl ClassDecl {
    ...
    /// (cross-crate via the `test-util` feature).
    #[cfg(any(test, feature = "test-util"))]
    pub fn for_test(name: &str) -> Self {
        Self::empty_named(name.to_string())
    }
    ...
    pub fn initial_enum_kind(&self) -> crate::types::EnumKind {
        if self.is_key_enum {
...
pub struct AnnotationBlock {
    ...
    pub alias_type_param_constraints: Vec<Option<String>>,
    pub alias_is_opaque: bool,
    ...
    pub creates_global: Option<crate::annotations::CreatesGlobalSpec>,
    /// `@generates-events N [FieldName]` — see [`crate::annotations::GeneratesEventsSpec`].
    pub generates_events: Option<crate::annotations::GeneratesEventsSpec>,
    ...
    pub event_batch_entries: Vec<(String, Vec<crate::pre_globals::EventPayloadParam>, usize)>,
...
pub fn extract_class_from_field_comments(field_node: SyntaxNode<'_>) -> Option<(String, u32)> {
    use crate::syntax::tree::SyntaxToken;
...
pub use scan_callback::{scan_callback_registries, build_generates_events_methods, build_generates_events_methods_iter};
