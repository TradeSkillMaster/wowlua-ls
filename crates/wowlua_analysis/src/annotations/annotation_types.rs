...
pub fn substitute_alias_type_params(
    body: &AnnotationType,
    type_params: &[String],
    args: &[AnnotationType],
) -> AnnotationType {
...
pub fn detect_event_params_from_generic(
...
pub fn parse_type(s: &str) -> AnnotationType {
    let s = s.trim();
...
pub type ReturnEntry = (AnnotationType, Option<String>, Option<String>);
...
pub fn parse_multi_return_line(s: &str) -> Option<Vec<ReturnEntry>> {
...
pub fn parse_return_line(s: &str, force_tuple: bool) -> (AnnotationType, Option<String>, Option<String>) {
    let s = s.trim();
    if s.starts_with('(') {
...
pub fn strip_return_description(s: &str) -> &str {
    let bytes = s.as_bytes();
    let mut depth = 0usize;
    let mut end = s.len();
...
pub fn split_at_top_level(s: &str, sep: char) -> Vec<&str> {
