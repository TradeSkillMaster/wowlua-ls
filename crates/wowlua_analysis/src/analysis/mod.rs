...
/// Map a resolved `ValueType` back to its annotation-style type name (used when a
/// backtick `@return \`K\`` must materialize the bound type's name as a string
/// literal). Lives here rather than in `annotations` so the shared annotation
/// types never need to reach back into the analysis IR (`Ir`).
pub fn value_type_to_name(vt: &ValueType, ir: &Ir) -> Option<String> {
    match vt {
...
pub fn ancestor_scopes(scopes: &[Scope], start: ScopeIndex) -> impl Iterator<Item = ScopeIndex> + '_ {
...
pub struct NarrowingState {
    pub narrowed: HashMap<ScopeIndex, HashSet<NarrowTarget>>,
    pub falsy_narrowed: HashMap<ScopeIndex, HashSet<NarrowTarget>>,
    pub truthy_narrowed_symbols: HashMap<ScopeIndex, HashSet<SymbolIndex>>,
    pub class_narrowed_symbols: HashMap<ScopeIndex, HashMap<SymbolIndex, String>>,
    ...
    pub num_compare_narrowed_symbols: HashMap<ScopeIndex, HashMap<SymbolIndex, (crate::ast::Operator, String)>>,
    ...
    pub type_narrowed: HashMap<ScopeIndex, HashMap<NarrowTarget, ValueType>>,
    /// Strip-style type narrowing for the inverse type() guard
    /// (`if type(obj.f) == "table" then` → else-branch strips table).
    pub type_stripped: HashMap<ScopeIndex, HashMap<NarrowTarget, ValueType>>,
    ...
    pub type_filtered_symbols: HashMap<ScopeIndex, HashMap<SymbolIndex, ValueType>>,
    ...
    pub narrowing_overridden: HashMap<ScopeIndex, HashMap<SymbolIndex, u32>>,
    ...
    pub field_assignment_narrowed: HashMap<ScopeIndex, HashMap<NarrowTarget, crate::types::ExprId>>,
    ...
    pub falsy_narrowed_pre_reassign: HashSet<SymbolIndex>,
    ...
    pub assert_implications: HashMap<ScopeIndex, Vec<AssertImplication>>,
}
...
#[derive(Clone)]
pub struct AssertImplication {
    pub antecedent: SymbolIndex,
    pub antecedent_ver: usize,
    pub consequent: SymbolIndex,
    pub consequent_ver: usize,
}

impl NarrowingState {
    pub fn is_symbol_narrowed(&self, scopes: &[Scope], sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> bool {
        scope_set_contains(&self.narrowed, scopes, &NarrowTarget::Symbol(sym_idx), scope_idx)
    }

    pub fn is_symbol_falsy_narrowed(&self, scopes: &[Scope], sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> bool {
        scope_set_contains(&self.falsy_narrowed, scopes, &NarrowTarget::Symbol(sym_idx), scope_idx)
    }

    pub fn is_symbol_truthy_narrowed(&self, scopes: &[Scope], sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> bool {
        scope_set_contains(&self.truthy_narrowed_symbols, scopes, &sym_idx, scope_idx)
    }

    pub fn get_type_narrowing(&self, scopes: &[Scope], sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> Option<&ValueType> {
        scope_map_get(&self.type_narrowed, scopes, &NarrowTarget::Symbol(sym_idx), scope_idx)
    }

    pub fn get_type_filtering(&self, scopes: &[Scope], sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> Option<&ValueType> {
        scope_map_get(&self.type_filtered_symbols, scopes, &sym_idx, scope_idx)
    }

    pub fn get_type_stripping(&self, scopes: &[Scope], sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> Option<&ValueType> {
        scope_map_get(&self.type_stripped, scopes, &NarrowTarget::Symbol(sym_idx), scope_idx)
    }

    pub fn get_field_type_narrowing(&self, scopes: &[Scope], sym_idx: SymbolIndex, chain: &[String], scope_idx: ScopeIndex) -> Option<&ValueType> {
    ...
    pub fn is_field_chain_narrowed(&self, scopes: &[Scope], sym_idx: SymbolIndex, chain: &[String], scope_idx: ScopeIndex) -> bool {
    ...
    pub fn is_field_falsy_narrowed(&self, scopes: &[Scope], sym_idx: SymbolIndex, chain: &[String], scope_idx: ScopeIndex) -> bool {
    ...
    pub fn get_field_assignment_narrow_rhs(&self, scopes: &[Scope], sym_idx: SymbolIndex, chain: &[String], scope_idx: ScopeIndex) -> Option<crate::types::ExprId> {
    ...
    pub fn is_narrowing_overridden(&self, scopes: &[Scope], sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> bool {
    ...
    pub fn is_narrowing_overridden_at(&self, scopes: &[Scope], sym_idx: SymbolIndex, scope_idx: ScopeIndex, at_offset: u32) -> bool {
        let override_scope = ancestor_scopes(scopes, scope_idx)
...
pub struct Ir {
    ...
    pub ext: Arc<PreResolvedGlobals>,
    ...
    // (diagnostics in this crate; doc_gen, stub_gen, the LSP main loop, plugins in
    // higher crates) must read through the routing/iterator surface above
    // (`sym`/`func`/`expr`/`table`/`scope`, `local_*`, `get_symbol`,
    // `scope_at_offset`, `ancestor_scopes`, `scope0_*`) — the crate's `pub` API now.
    // Keep these fields private; do not widen them.
    scopes: Vec<Scope>,
    symbols: Vec<Symbol>,
    ...
    pub parameterized_aliases: HashMap<String, (Vec<String>, crate::annotations::AnnotationType)>,
    ...
    pub parameterized_alias_constraints: HashMap<String, Vec<Option<(String, crate::annotations::AnnotationType)>>>,
    ...
    pub class_table_by_offset: HashMap<u32, TableIndex>,
    ...
    pub class_def_symbols: HashSet<SymbolIndex>,
    ...
    pub lateinit_symbols: HashSet<SymbolIndex>,
    /// Scope in which each VarArgs expression was created (for event-param narrowing).
    pub varargs_scope: HashMap<ExprId, ScopeIndex>,
    ...
    pub event_type_display: HashMap<(SymbolIndex, usize), String>,
    ...
    pub expression_args: HashMap<ExprId, ExpressionArg>,
    ...
    pub synthesized_overload_funcs: HashSet<FunctionIndex>,
    ...
    pub pending_bracket_assigns: Vec<(String, ScopeIndex, ExprId)>,
...
impl Ir {
    ...
    pub fn record_loop_condition_site(&mut self, expr_id: ExprId, range: crate::syntax::tree::TextRange, loop_scope: ScopeIndex) {
        self.condition_sites.push(ConditionSite {
            expr_id,
            start: u32::from(range.start()),
            end: u32::from(range.end()),
    ...
    #[inline]
    pub fn addon_table_idx(&self) -> Option<TableIndex> {
    ...
    #[inline]
    pub fn is_stub_function(&self, func_idx: FunctionIndex) -> bool {
    ...
    #[inline]
    pub fn is_global_env(&self, table_idx: TableIndex) -> bool {
    ...
    pub fn sym(&self, idx: SymbolIndex) -> &Symbol {
        if idx.is_external() {
    ...
    pub fn func(&self, idx: FunctionIndex) -> &Function {
        if idx.is_external() {
    ...
    pub fn type_var_class_constraint_for_param(
        &self,
    ...
    pub fn expr(&self, idx: ExprId) -> &Expr {
        if idx.is_external() {
            self.ext.expr(idx)
        } else {
            &self.exprs[idx.val()]
    ...
    #[inline]
    pub fn try_expr(&self, idx: ExprId) -> Option<&Expr> {
        if idx.is_external() {
    ...
    pub fn table(&self, idx: TableIndex) -> &TableInfo {
        if idx.is_external() {
            self.ext.table(idx)
        } else {
            &self.tables[idx.val()]
    ...
    #[inline]
    pub fn local_symbols(&self) -> impl Iterator<Item = (SymbolIndex, &Symbol)> {
    ...
    #[inline]
    pub fn local_functions(&self) -> impl Iterator<Item = (FunctionIndex, &Function)> {
    ...
    #[inline]
    pub fn local_tables(&self) -> impl Iterator<Item = (TableIndex, &TableInfo)> {
    ...
    #[inline]
    pub fn local_exprs(&self) -> impl Iterator<Item = (ExprId, &Expr)> {
    ...
    #[inline]
    pub fn local_scopes(&self) -> impl Iterator<Item = (ScopeIndex, &Scope)> {
    ...
    #[inline]
    pub fn scope0_local_symbols(&self) -> impl Iterator<Item = (&SymbolIdentifier, SymbolIndex)> {
    ...
    #[inline]
    pub fn scope0_global_symbol(&self, id: &SymbolIdentifier) -> Option<SymbolIndex> {
        self.scopes.first()
    ...
    pub fn external_class_table_for(&self, name: &str) -> Option<TableIndex> {
    ...
    pub fn external_table_for(&self, name: &str) -> Option<TableIndex> {
    ...
    #[inline]
    pub fn scope(&self, idx: ScopeIndex) -> &Scope {
        if idx.is_external() {
            self.ext.scope(idx)
        } else {
            &self.scopes[idx.val()]
    ...
    #[inline]
    pub fn try_scope(&self, idx: ScopeIndex) -> Option<&Scope> {
        if idx.is_external() {
    ...
    #[inline]
    pub fn ancestor_scopes(&self, start: ScopeIndex) -> impl Iterator<Item = ScopeIndex> + '_ {
    ...
    pub fn explicit_map_value_type(&self, table_indices: &[TableIndex]) -> Option<ValueType> {
    ...
    pub fn is_open_literal_union_symbol(&self, sym_idx: SymbolIndex) -> bool {
    ...
    pub fn get_symbol(&self, id: &SymbolIdentifier, scope_idx: ScopeIndex) -> Option<SymbolIndex> {
    ...
    pub fn get_symbol_excluding(&self, id: &SymbolIdentifier, scope_idx: ScopeIndex, exclude: SymbolIndex) -> Option<SymbolIndex> {
    ...
    pub fn library_table_for_type(&self, vt: &ValueType) -> Option<TableIndex> {
    ...
    pub fn collect_library_table_indices(&self, vt: &ValueType, indices: &mut Vec<TableIndex>) {
        match vt {
    ...
    pub fn is_anonymous_empty_table(&self, idx: TableIndex) -> bool {
        let t = self.table(idx);
    ...
    pub fn dedupe_union_tables(&self, vt: ValueType) -> ValueType {
    ...
    pub fn push_strip_nil_version(&mut self, sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> Option<usize> {
    ...
    /// Returns the new version index, or `None` if the symbol is external.
    pub fn push_overload_narrow_version(
    ...
    pub fn find_root_symbol(&self, expr_id: ExprId) -> Option<SymbolIndex> {
        match self.expr(expr_id) {
    ...
    pub fn extract_field_chain(&self, expr_id: ExprId) -> Option<(SymbolIndex, Vec<String>)> {
    ...
    pub fn resolve_constructor_func(&self, table_idx: TableIndex) -> Option<FunctionIndex> {
    ...
    pub fn get_field(&self, table_idx: TableIndex, field_name: &str) -> Option<&FieldInfo> {
    ...
    pub fn has_field(&self, table_idx: TableIndex, field_name: &str) -> bool {
    ...
    /// (`{a} | {b}`) are preserved.
    pub fn collapse_subset_tables(&self, types: Vec<ValueType>) -> Vec<ValueType> {
    ...
    pub fn has_accessor(&self, table_idx: TableIndex, name: &str) -> bool {
    ...
    pub fn is_scope_visible_from(&self, version_scope: ScopeIndex, reference_scope: ScopeIndex) -> bool {
    ...
    pub fn version_for_scope(&self, sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> usize {
    ...
    pub fn is_scope_eq_or_descendant(&self, scope: ScopeIndex, container: ScopeIndex) -> bool {
    ...
    pub fn version_for_scope_ancestors_only(&self, sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> usize {
        if sym_idx.is_external() {
            return self.ext.sym(sym_idx).versions.len() - 1;
        }
        let sym = &self.symbols[sym_idx.val()];
    ...
    pub fn version_for_scope_ancestors_with_order(&self, sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> usize {
        if sym_idx.is_external() {
            return self.ext.sym(sym_idx).versions.len() - 1;
        }
        let scope_order = self.scopes[scope_idx.val()].creation_order;
    ...
    pub fn insert_overlay_field(&mut self, table_idx: TableIndex, field_name: String, field_info: FieldInfo) {
        self.overlay_fields.entry(table_idx).or_default().insert(field_name, field_info);
    }

    /// Get a mutable reference to an overlay field.
    pub fn get_overlay_field_mut(&mut self, table_idx: TableIndex, field_name: &str) -> Option<&mut FieldInfo> {
        self.overlay_fields.get_mut(&table_idx)?.get_mut(field_name)
    }

    // ── Methods shared by Analysis and AnalysisResult ────────────────────────

    pub fn scope_at_offset(&self, offset: impl Into<u32>) -> Option<ScopeIndex> {
    ...
    pub fn function_name(&self, func_idx: FunctionIndex) -> Option<String> {
    ...
    pub fn same_class(&self, a: TableIndex, b: TableIndex) -> bool {
        if a == b { return true; }
    ...
    pub fn find_enclosing_class(&self, node: &SyntaxNode<'_>) -> Option<TableIndex> {
    ...
    pub fn check_annotation_type_names(
        &self,
...
pub struct AnalysisResult {
    pub ir: Ir,
    pub is_meta: bool,
    pub symbol_version_at: HashMap<u32, usize>,
    pub resolved_expr_cache: Vec<Option<ValueType>>,
    /// Type-narrowing facts from control-flow guards (see `NarrowingState`).
    pub narrowing: NarrowingState,
    pub call_type_args: HashMap<ExprId, Vec<ValueType>>,
    pub field_type_args_cache: HashMap<(TableIndex, String), Vec<ValueType>>,
    ...
    pub method_decl_subs: HashMap<(u32, u32), HashMap<String, ValueType>>,
    pub referenced_symbols: HashSet<SymbolIndex>,
    pub inherited_constructors: HashSet<FunctionIndex>,
    pub function_owner_class: HashMap<FunctionIndex, String>,
    ...
    pub defclass_vars: HashMap<String, TableIndex>,
...
impl AnalysisResult {
    ...
    #[inline] pub fn sym(&self, idx: SymbolIndex) -> &Symbol { self.ir.sym(idx) }
    #[inline] pub fn func(&self, idx: FunctionIndex) -> &Function { self.ir.func(idx) }
    #[inline] pub fn expr(&self, idx: ExprId) -> &Expr { self.ir.expr(idx) }
    #[inline] pub fn table(&self, idx: TableIndex) -> &TableInfo { self.ir.table(idx) }
    #[inline] pub fn scope(&self, idx: ScopeIndex) -> &Scope { self.ir.scope(idx) }
    #[inline] pub fn try_scope(&self, idx: ScopeIndex) -> Option<&Scope> { self.ir.try_scope(idx) }
    #[inline] pub fn local_symbols(&self) -> impl Iterator<Item = (SymbolIndex, &Symbol)> { self.ir.local_symbols() }
    #[inline] pub fn local_functions(&self) -> impl Iterator<Item = (FunctionIndex, &Function)> { self.ir.local_functions() }
    #[inline] pub fn local_tables(&self) -> impl Iterator<Item = (TableIndex, &TableInfo)> { self.ir.local_tables() }
    ...
    #[inline] pub fn same_class(&self, a: TableIndex, b: TableIndex) -> bool { self.ir.same_class(a, b) }
    #[inline] pub fn is_subclass_of(&self, child_idx: TableIndex, parent_idx: TableIndex) -> bool { self.ir.is_subclass_of(child_idx, parent_idx) }
    #[inline] pub fn find_enclosing_class(&self, node: &SyntaxNode<'_>) -> Option<TableIndex> { self.ir.find_enclosing_class(node) }
    ...
    #[inline] pub fn resolve_constructor_func(&self, table_idx: TableIndex) -> Option<FunctionIndex> { self.ir.resolve_constructor_func(table_idx) }

    pub fn resolve_class_constraint(&self, constraint_str: &str) -> Option<ValueType> {
...
#[derive(Debug, Clone)]
pub struct SynthOverloadRefinement {
...
pub struct Analysis<'a> {
    ...
    pub deferred_field_mixins: Vec<crate::types::DeferredFieldMixin>,
    // Metadata (written during build_ir, read during resolve+checks)
    pub defclass_vars: HashMap<String, TableIndex>,
    /// Type-narrowing facts from control-flow guards (see `NarrowingState`).
    pub narrowing: NarrowingState,
    pub type_of_aliases: HashMap<SymbolIndex, SymbolIndex>,
    /// Maps `local b = type(x) == "typename"` → (target_sym, type_name, is_positive).
    /// `is_positive` is true for `==`, false for `~=`.
    pub type_guard_aliases: HashMap<SymbolIndex, (SymbolIndex, String, bool)>,
    pub symbol_version_at: HashMap<u32, usize>, // token start offset → version_idx used at that point
    ...
    pub sym_ref_sites: HashMap<SymbolIndex, Vec<(ExprId, u32)>>,
    pub referenced_symbols: HashSet<SymbolIndex>,
    pub functions_with_returns: HashSet<FunctionIndex>,
    ...
    pub resolving_exprs: Vec<bool>,
    pub resolve_depth: usize,
    pub resolve_work_count: usize,
    ...
    pub builder_call_memo: HashMap<ExprId, TableIndex>,
    ...
    pub call_site_generic_subs: HashMap<ExprId, HashMap<String, ValueType>>,
    ...
    pub multi_return_siblings: HashMap<SymbolIndex, Vec<(usize, SymbolIndex)>>,
    ...
    pub deferred_sibling_narrowings: Vec<DeferredSiblingNarrowing>,
    ...
    pub deferred_class_eq_narrowings: Vec<(SymbolIndex, ExprId, ScopeIndex)>,
    ...
    pub deferred_event_narrowings: Vec<(SymbolIndex, String, ScopeIndex)>,
    ...
    pub correlated_locals: Vec<Vec<SymbolIndex>>,
    ...
    pub guard_implications: Vec<(Vec<SymbolIndex>, SymbolIndex, ScopeIndex)>,
    ...
    pub and_or_num_sentinel: HashMap<SymbolIndex, (SymbolIndex, f64)>,
    ...
    pub conditionally_reached_exprs: HashSet<ExprId>,
...
impl<'a> Analysis<'a> {
    ...
    pub fn is_symbol_truthy_narrowed(&self, sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> bool {
    ...
    pub fn get_field_type_stripping(&self, sym_idx: SymbolIndex, chain: &[String], scope_idx: ScopeIndex) -> Option<&ValueType> {
    ...
    pub fn is_narrowing_overridden(&self, sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> bool {
    ...
    pub fn is_field_falsy_narrowed(&self, sym_idx: SymbolIndex, fields: &[String], scope_idx: ScopeIndex) -> bool {
        self.narrowing.is_field_falsy_narrowed(&self.ir.scopes, sym_idx, fields, scope_idx)
    }

    pub fn get_field_assignment_narrow_rhs(&self, sym_idx: SymbolIndex, fields: &[String], scope_idx: ScopeIndex) -> Option<crate::types::ExprId> {
...
pub fn type_involves_type_variable_impl(ir: &Ir, vt: &ValueType) -> bool {
    match vt {
        ValueType::TypeVariable(_) => true,
        ValueType::Table(Some(idx)) => {
...
pub fn fields_structurally_match_impl(
    ir: &Ir,
    resolved_expr_cache: &[Option<ValueType>],
...
pub enum StructuralMismatchDetail {
    Missing { field: String },
    WrongType { field: String, expected: ValueType, actual: ValueType },
}

pub fn collect_class_fields_impl(
    ir: &Ir,
    resolved_expr_cache: &[Option<ValueType>],
