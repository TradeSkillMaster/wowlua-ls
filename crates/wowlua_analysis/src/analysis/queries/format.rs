...
pub fn return_type_at_slot(ir: &Ir, rets: &[SymbolIndex], slot: usize) -> Option<ValueType> {
...
pub fn dedup_return_types(ir: &Ir, rets: &[SymbolIndex]) -> Vec<Option<ValueType>> {
...
pub fn format_vararg_return(formatted: String, index: usize, func: &Function) -> String {
...
pub fn format_vararg_param(ann: &crate::annotations::AnnotationType) -> String {
...
impl AnalysisResult {
    ...
    pub fn format_see_doc(&self, see: &[String]) -> Option<String> {
        if see.is_empty() {
            None
        } else {
    ...
    pub fn format_type(&self, vt: &ValueType) -> String {
        self.format_type_depth(vt, 0)
    }

    pub fn get_type_args_for_expr(&self, expr_id: ExprId) -> Vec<ValueType> {
    ...
    pub fn format_type_depth(&self, vt: &ValueType, depth: usize) -> String {
    ...
    pub fn format_value_type_depth(&self, vt: &ValueType, depth: usize) -> String {
    ...
    pub fn is_param_symbol(&self, symbol_idx: SymbolIndex) -> bool {
        if symbol_idx.is_external() {
            return false;
        }
    ...
    pub fn is_stub_symbol(&self, symbol_idx: SymbolIndex) -> bool {
    ...
    pub fn format_inferred_returns(&self, func: &Function, depth: usize) -> Vec<String> {
    ...
    /// `number?, number?`).
    pub fn return_only_overload_summary(return_only: &[&ResolvedOverload]) -> Vec<ValueType> {
    ...
    pub fn inferred_return_types(&self, func: &Function) -> Vec<ValueType> {
        let return_only: Vec<&ResolvedOverload> = func.overloads.iter()
            .filter(|o| o.is_return_only).collect();
        if !return_only.is_empty() {
            return Self::return_only_overload_summary(&return_only);
