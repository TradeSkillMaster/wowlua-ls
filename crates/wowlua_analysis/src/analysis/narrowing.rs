...
impl<'a> Analysis<'a> {
    ...
    pub fn sibling_was_reassigned(&self, sibling_idx: SymbolIndex, scope_idx: ScopeIndex, expected_ret_index: usize) -> bool {
    ...
    pub fn collect_narrowed_sibling_info(&self, siblings: &[(usize, SymbolIndex)], scope_idx: ScopeIndex) -> Vec<(usize, NarrowKind)> {
    ...
    pub fn narrow_kind_for(&self, sym_idx: SymbolIndex, scope_idx: ScopeIndex) -> Option<NarrowKind> {
    ...
    pub fn check_return_only_overloads_from_siblings(&self, siblings: &[(usize, SymbolIndex)]) -> OverloadCheck {
    ...
    /// picking up versions from descendant scopes (e.g. then-branch versions
    /// that would corrupt the result in early-exit narrowing).
    pub fn push_type_filter_version(&mut self, sym_idx: SymbolIndex, guard_type: ValueType, scope_idx: ScopeIndex, ancestors_only: bool) {
        if !sym_idx.is_external() {
            let prev_ver = if ancestors_only {
                self.ir.version_for_scope_ancestors_only(sym_idx, scope_idx)
            } else {
