...
pub fn trimmed_node_end(node: SyntaxNode<'_>) -> u32 {
...
/// A work-item in `build_ir`'s explicit statement-processing stack: a block whose
/// statements are walked one at a time, carrying the scope/function context in
/// which they execute.
#[derive(Clone, Copy)]
struct Frame<'a> {
    block: Block<'a>,
...
impl<'a> Analysis<'a> {
    pub(super) fn build_ir(&mut self) {
        ...
        while let Some(frame) = stack.last_mut() {
            ...
            match &statements[stmt_index] {
                Statement::LocalAssign(assign) => self.build_stmt_local_assign(assign, scope_idx, func_id, &mut stack),
                ...
                Statement::Return(ret) => self.build_stmt_return(ret, scope_idx, func_id),
                Statement::Assign(assign) => self.build_stmt_assign(assign, scope_idx, func_id, constructor_of, stmt_index, &mut stack),
                Statement::FunctionCall(call) => self.build_stmt_function_call(call, scope_idx),
            }
    ...
    fn build_stmt_local_assign(&mut self, assign: &LocalAssign<'a>, scope_idx: ScopeIndex, func_id: Option<FunctionIndex>, stack: &mut Vec<Frame<'a>>) {
    ...
    fn build_stmt_do(&mut self, group: &DoGroup<'a>, scope_idx: ScopeIndex, func_id: Option<FunctionIndex>, constructor_of: Option<TableIndex>, frame_is_conditional: bool, stack: &mut Vec<Frame<'a>>) {
    ...
    fn build_stmt_while(&mut self, while_loop: &WhileLoop<'a>, scope_idx: ScopeIndex, func_id: Option<FunctionIndex>, constructor_of: Option<TableIndex>, stack: &mut Vec<Frame<'a>>, pending_while_narrowings: &mut Vec<PendingWhileNarrowing>) {
    ...
    fn build_stmt_for_count(&mut self, for_loop: &ForCountLoop<'a>, scope_idx: ScopeIndex, func_id: Option<FunctionIndex>, constructor_of: Option<TableIndex>, stack: &mut Vec<Frame<'a>>) {
    ...
    fn build_stmt_for_in(&mut self, for_in: &ForInLoop<'a>, scope_idx: ScopeIndex, func_id: Option<FunctionIndex>, constructor_of: Option<TableIndex>, stack: &mut Vec<Frame<'a>>) {
    ...
    fn build_stmt_function_definition(&mut self, func: &FunctionDefinition<'a>, scope_idx: ScopeIndex, func_id: Option<FunctionIndex>, stmt_index: usize, stack: &mut Vec<Frame<'a>>) {
    ...
    fn build_stmt_return(&mut self, ret: &Return<'a>, scope_idx: ScopeIndex, func_id: Option<FunctionIndex>) {
    ...
    /// in `build_ir` for why this runs before the pop check.
    fn process_pending_branch_merges(&mut self, scope_idx: ScopeIndex, pending_branch_merges: &mut Vec<PendingBranchMerge>) {
        let mut mi = 0;
    ...
    /// Run end-of-block cleanup after a frame is popped: synthesize correlated
    /// return-only overloads for a finished function body, apply pending while-loop
    /// exit narrowings, and forward do-block / for-loop reassignments to the parent
    /// scope. `stack` is the post-pop stack (its top is the resumed parent frame).
    fn finalize_popped_frame(&mut self, popped_block: Block<'a>, popped_scope: ScopeIndex, popped_func_id: Option<FunctionIndex>, stack: &[Frame<'a>], pending_while_narrowings: &mut Vec<PendingWhileNarrowing>) {
    ...
    pub fn collect_preceding_annotation_ranges(node: SyntaxNode<'_>) -> Vec<(String, usize, usize)> {
    ...
    pub fn extract_inline_type(field_node: SyntaxNode<'_>) -> Option<AnnotationType> {
