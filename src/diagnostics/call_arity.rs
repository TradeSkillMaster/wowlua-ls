        ...
        if actual_count > expected_count && !is_vararg && !last_is_multi {
                ...
                && let Some(&(start, end)) = arg_ranges.get(expected_count)
            {
                diags.push(WowDiagnostic {
                    code: super::redundant_param::CODE,
                    message: format!("expected at most {} argument(s) but got {}", expected_count, actual_count),
            ...
            if actual_count < required_count {
                ...
                if !overload_satisfied {
                    ...
                    if let Some(name) = param_name {
                        diags.push(WowDiagnostic {
                            code: super::missing_param::CODE,
                            message: format!("missing argument for parameter '{}'", name),
                            severity: DiagnosticSeverity::WARNING,
