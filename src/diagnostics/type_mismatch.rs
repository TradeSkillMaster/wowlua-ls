...
pub(crate) fn run_arg_checks(analysis: &AnalysisResult, excess_inject: &mut Vec<InjectFieldCheck>, diags: &mut Vec<WowDiagnostic>) {
            ...
            if (!arg_type.is_assignable_to(&check.expected_type) && !structurally_matched)
                ...
                if is_nil_union_compatible {
                    ...
                    diags.push(WowDiagnostic {
                        code: CODE,
                        message: format!("expected `{}` for parameter '{}', got `{}`", expected_str, check.param_name, actual_str),
