...
pub(crate) fn run(analysis: &AnalysisResult, excess_inject: &mut Vec<InjectFieldCheck>, diags: &mut Vec<WowDiagnostic>) {
    for fa in &analysis.ir.field_assignments {
        ...
        diags.push(WowDiagnostic {
            code: CODE,
            message: format!("expected `{}` for field '{}', got `{}`", expected_str, fa.field_name, actual_str),
            severity: DiagnosticSeverity::WARNING,
            start: fa.expr_start as usize,
            end: fa.expr_end as usize,
        });
    }
}
