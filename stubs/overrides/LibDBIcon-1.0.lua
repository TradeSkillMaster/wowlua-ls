---@meta _
-- Override of Ketho's LibDBIcon-1.0 annotations. Identical to the upstream
-- vendor file except that the `LibDBIcon.button.DB` fields are marked optional:
-- the SavedVariable `db` table passed to `LibDBIcon:Register(name, object, db)`
-- is a partial options table (the library fills in defaults for anything that is
-- absent), so a valid call like `LibDBIcon:Register(name, obj, { hide = false })`
-- must not report `missing-fields` for the omitted `lock`/`minimapPos`.
---@class LibDBIcon-1.0
...
-- All fields optional: the `db` SavedVariable is a partial options table and the
-- library supplies defaults for anything absent.
